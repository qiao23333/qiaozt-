...
然后浏览器打开 **http://localhost:4323** —— 这是一个专门给本站做的中文写作工作台：
填标题/日期/标签等表单、正文可视化编辑、保存后直接生成文章文件到 `src/content/posts/`，
不用手动碰任何 Markdown 文件。（用完关掉终端即可。）
...
### 方式 C：上线后浏览器后台（Sveltia CMS，待部署时启用）

网站部署到 Vercel/Cloudflare Pages 后，可以接入 **Sveltia CMS**（开源免费，中文界面）：
在浏览器打开 `你的域名/admin`，像用公众号后台一样写文章、传图片、管理草稿，
保存即自动提交到 GitHub 并触发重新部署，**手机上也能写**。部署时告诉我启用即可。

三种方式产物都是同一批文章文件，可混用，不冲突。
...
网站的外观由三层组成，**访客端切换、自动保存在访客浏览器（localStorage）**，下次访问自动恢复：
...
### 整站多主题（换一套完全不同的页面结构/UI）

当前主题 = 主题 1（Firefly 二改），定稿状态已用 git 标签存档。
想开新主题时对我说**「开始创建新的主题」**即可，流程见 [THEMES.md](THEMES.md)：
旧主题自动冻结保存（分支备份），新主题在共享内容的基础上开发，随时可切回。
