...
description: 用于验证 Rain 中 PlantUML 插件渲染、主题切换与交互能力的示例文章。
aiSummary: 这篇指南用活动图、状态图、用例图、组件图、部署图、ER 图、时序图和 C4 容器图说明 PlantUML 的文本化建模方式，以及 Rain 的主题切换与交互。
tags: [PlantUML, Rain, Markdown]
category: 文章示例
draft: true
---
...
在 Rain 中，`plantuml` 代码块会在构建阶段编码并生成服务器 SVG 地址，页面端再根据亮暗主题自动切换图源，并支持缩放、拖拽和全屏交互。
...
System_Boundary(system, "Rain Blog") {
